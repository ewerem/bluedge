...
<div class="uk-container">
	...
	<center>
		...
		<h5 style="padding-bottom: 5px;">Academic/Career Form</h5>
	</center>
	<br>
	...
	<form method="post">
		...
		<i style="color:black; font-size:16px;">Your Skill</i><br>
        <input type="text" name="skill" id="field" class="uk-input uk-form-large" required="" style="padding: 5px;color:black;border:1px solid #ccc; width:100%;border-bottom:3px solid black !important;font-size: 16px;">
        <br><br>

        <i style="color:black; font-size:16px;">Qualification</i><br>
        <select name="qual" id="field" class="uk-select uk-form-large" required="" style="padding: 5px;color:black;border:1px solid #ccc; width:100%;border-bottom:3px solid black !important;font-size: 16px;">
...
<?php  
	...
	if(isset($_POST['subform'])){
		...
		$ins = $pdo->prepare("INSERT INTO academic (name, phone, email, address, qualification, skill, aim, time) VALUES(?,?,?,?,?,?,?,CURRENT_TIMESTAMP)");
		$ins->bindParam(1, $fna);
		$ins->bindParam(2, $phn);
		$ins->bindParam(3, $ema);
		$ins->bindParam(4, $had);
		...
		if($ins){
			echo'<script>alert("THANK YOU, WE HAVE RECIEVED YOUR INFORMATION");</script>';
		}else{
